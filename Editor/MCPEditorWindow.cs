...
namespace MCP4Unity.Editor
{
    ...
    public class MCPEditorWindow : EditorWindow, ISerializationCallbackReceiver
    {
        ...
        {
            bool running = MCPService.Inst.Running;
            if (startBtn != null)
            {
            ...
            if (portLabel != null)
            {
                int port = MCPService.Inst.CurrentPort;
                portLabel.text = running ? $"Port: {port}" : $"Port: {port} (Stopped)";
                portLabel.style.color = running ? Color.green : Color.gray;
            }
            titleContent.text = "MCP:" + (running ? "Running" : "Stopped");
        }
