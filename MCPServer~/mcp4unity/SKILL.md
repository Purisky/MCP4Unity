...
```
configureunity unityExePath="path/to/Unity/Editor/Unity.exe"
```

可选参数：
- `projectPath`: 指定项目路径（默认自动检测）
- `mcpPort`: Unity Editor MCP 服务端口（默认 52429）

多项目配置示例：
```
# 项目 A 使用默认端口 52429
configureunity unityExePath="..." projectPath="path/to/projectA"

# 项目 B 使用自定义端口 52430（避免冲突）
configureunity unityExePath="..." projectPath="path/to/projectB" mcpPort=52430
```
...
**重要**: 检查编译错误时，优先使用 batchmode 而不是 Editor 模式。batchmode 更可靠，不会因为 MCP 服务未启动而卡住。

```
编译检查流程：
1. stopunity（如果 Unity 正在运行）
2. runbatchmode（通过 MCP 工具启动 batchmode 编译）
3. 检查结构化输出：
   - ✅ 编译成功 → 完成
   - ❌ 编译失败 → 查看前 20 条错误
4. 修复错误后重复步骤 2-3
...
- 成功/失败状态
- 错误和警告统计
- 前 20 条错误详情（文件:行号 + 错误信息）
- 前 10 条警告详情
- 完整日志路径（仅在需要时查阅）

Editor 模式编译（仅在 batchmode 不可用时使用）：
getunitystatus →
...
**禁止**: 不要使用命令行直接调用 Unity.exe -batchmode，必须通过 MCP 工具。
...
| `runbatchmode` | 启动 Unity batchmode 编译（推荐用于检查编译错误） |
| `stopunity` | 强制关闭 Unity |
| `getunitystatus` | 检查 Unity 状态（简洁或详细模式） |
...
`getunitystatus` 支持两种输出模式：

### 简洁模式（默认）

```
getunitystatus  # 或 getunitystatus(detailed=false)
```
...
**工具位置**:
- **内置工具**: `Assets/MCP4Unity/Editor/Tools/` (框架默认能力)
- **自定义工具**: `Assets/Editor/MCPTools/` (用户扩展)
...
├── Library/
│   └── MCP4Unity/
│       └── mcp_endpoint.json  # Unity MCP 服务端点（Unity 自动生成）
└── unity_config.json          # Unity 路径配置（configureunity 自动创建）
```

**注意**: `unity_config.json` 存储在 Unity 项目根目录（包含 Assets/ 和 ProjectSettings/ 的目录），支持多个 Unity 项目各自独立配置。
